...
enum class OrderSide : int { UNDEFINED = 0, BUY = 1, SELL = 2 };
...
struct SimpleTimeAndSaleDataProvider {
  ...
  static std::future<std::unordered_map<std::string, std::vector<TimeAndSale>>> run(
    const std::string &address, const std::vector<std::string> &symbols) {
    return std::async(std::launch::async, [address, symbols]() {
      struct Impl {
...
int main(int argc, char *argv[]) {
  ...
  for (auto [s, v] : SimpleTimeAndSaleDataProvider::run(argv[1], {"/ESZ21:XCME", "/FESX211217:XEUR", "AAPL"}).get()) {
    std::cout << s << "[" << v.size() << "]\n";
  }

  auto f = SimpleTimeAndSaleDataProvider::run(argv[1], {"/ESZ21:XCME", "/FESX211217:XEUR", "AAPL"});
  auto f2 = SimpleTimeAndSaleDataProvider::run(argv[2], {"/ESZ21:XCME", "/FESX211217:XEUR", "AAPL"});
